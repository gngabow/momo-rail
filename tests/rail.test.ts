...
describe.each(MARKETS)('rail end-to-end · $country ($currency)', (m) => {
  test('deposit credits the local wallet', async () => {
    const { ledger, rail } = await makeRail();
    ...
    expect((await ledger.getBalance(dep.walletId!)).balance).toBe(
  ...
  test('convert local -> USDT lands the same 98.5 USDT on both markets', async () => {
    ...
    expect((await ledger.getBalance(cvt.usdtWalletId)).balance).toBe('98.500000');
  });
  ...
  test('withdraw disburses and debits the local wallet', async () => {
    ...
    const walletBal = (await ledger.getBalance((await ledger.findCustomerWallet('c1', m.currency))!.id)).balance;
...
describe('rail guards', () => {
  test('a declined MoMo prompt leaves no balance', async () => {
    ...
    const w = await ledger.findCustomerWallet('c9', 'KES');
    expect(w?.id).toBeDefined();
    expect((await ledger.getBalance(w!.id)).balance).toBe('0.00');
  });
